...
Internalize the first Hermes-inspired runtime capability bundle into Forge CLI and keep canonical project docs in sync.

## Delivered

- Root governance files: `AGENTS.md`, `CLAUDE.md`, `CODEX.md`, `GEMINI.md`
- Git hygiene update: `.gitignore` now ignores `.omx/`
- Canonical project doc tree under `doc/00_project/initiative_forge_cli/`
...
- Model/provider ergonomics bundle:
  ...
  - `forge providers` now shows the same context/pricing/caution metadata before a session starts
- Centralized logging bundle:
  - `src/logger.ts` writes local runtime logs under `~/.armature/logs/` or `$ARMATURE_HOME/logs/`
  - `forge logs` and `forge logs errors` surface recent log entries
...
- Doctor diagnostics bundle:
  ...
  - `forge doctor` and `forge doctor --json` expose that state directly
...
- Serve observability bundle:
  - `forge serve` now reuses doctor/model-catalog metadata in `/health`, `/providers`, and `/doctor`
  - headless clients can inspect the same runtime state surfaces as CLI users
- Stats dashboard bundle:
  - `forge stats` now combines usage/cost, runtime health, and recent error summaries
- SDK boundary:
  - `MARUCIE-open-agent-sdk` reviewed repeatedly but still intentionally unchanged
  - Current Hermes-inspired slices remain Forge-local runtime ergonomics rather than shared provider-neutral SDK seams
- SDK boundary:
  - `MARUCIE-open-agent-sdk` reviewed but not changed; this bundle remains Forge-local for now
...
- Repo verification:
  ...
  - `OPENAI_API_KEY=test-openai-key ARMATURE_PROVIDER=openai node dist/bin/forge.js providers`
  - `ARMATURE_HOME=$(mktemp -d) node dist/bin/forge.js logs`
  - `node --experimental-vm-modules node_modules/.bin/vitest run tests/doctor-command.test.ts`
  - `OPENAI_API_KEY=test-openai-key ARMATURE_PROVIDER=openai node dist/bin/forge.js doctor --json`
